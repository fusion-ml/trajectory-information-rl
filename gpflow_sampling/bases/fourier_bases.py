...
from gpflow_sampling.bases.fourier_initializers import (
    bias_initializer,
    weight_initializer,
)
...
class Dense(AbstractFourierBasis):
    ...
    def initialize(self, x: TensorType, dtype: Any = None):
        ...
        self._weights = weight_initializer(
            self.kernel, x.shape[-1], batch_shape=[self.num_bases], dtype=dtype
        )
...
class MultioutputDense(Dense):
    def __call__(self, x: TensorType, multioutput_axis: int = None, **kwargs):
        ...
        proj = move_axis(
            batch_tensordot(self.weights, x, axes=[-1, -1], batch_axes=batch_axes),
            1,
            -1,
        )
    ...
    def initialize(self, x: TensorType, dtype: Any = None):
        ...
        for kernel in self.kernel.latent_kernels:
            biases.append(bias_initializer(kernel, self.num_bases, dtype=dtype))

            weights.append(
                weight_initializer(
                    kernel, x.shape[-1], batch_shape=[self.num_bases], dtype=dtype
                )
            )
    ...
    def weights(self):
        ...
        # [L, 1, D] or [L, 1, 1]
        inv_lengthscales = expand_to(
            arr=tf.stack(inv_lengthscales), axis=1, ndims=self._weights.shape.ndims
        )
...
class Conv2d(AbstractFourierBasis):
    ...
    def initialize(self, x, dtype: Any = None):
        ...
        weights = weight_initializer(
            self.kernel.kernel, patch_size, batch_shape=[self.num_bases], dtype=dtype
        )
...
class DepthwiseConv2d(Conv2d):
    ...
    def initialize(self, x, dtype: Any = None):
        ...
        weights = weight_initializer(
            self.kernel.kernel, patch_size, batch_shape=batch_shape, dtype=dtype
        )

        self._filters = tf.reshape(
            move_axis(weights, -1, 0), self.kernel.patch_shape + batch_shape
        )

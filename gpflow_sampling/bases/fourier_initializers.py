...
def _bias_initializer_fallback(
    kern: kernels.Stationary,
    ndims: int,
    *,
...
def _weight_initializer_squaredExp(
...
def _weight_initializer_matern(
    kern: MaternKernel,
    ndims: int,
    *,
    batch_shape: List = None,
    ...
    if gamma_rvs is None:
        gamma_rvs = tf.random.gamma(
            shape=batch_shape + [1], alpha=smoothness, beta=smoothness, dtype=dtype
        )
    else:

...
def _decoupled_fallback(
    ...
    **kwargs
    ...
    return CompositeSampler(
        samplers=[prior, update], join_rule=join_rule, mean_function=mean_function
    )


@decoupled.register(MultioutputKernel, AbstractSampler, TensorLike, TensorLike)
def _decoupled_multioutput(
    kern: MultioutputKernel,
    prior: AbstractSampler,
    Z: TensorLike,
    u: TensorLike,
    *,
    ...
    **kwargs
    ...
    if multioutput_axis_Z == "default":
        if isinstance(
            Z, inducing_variables.MultioutputInducingVariables
        ) and not isinstance(Z, inducing_variables.SharedIndependentInducingVariables):

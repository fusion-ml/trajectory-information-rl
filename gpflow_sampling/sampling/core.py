...
class DenseSampler(AbstractSampler):
    ...
    def __call__(self, x: tf.Tensor, sample_axis: int = "default", **kwargs):
        ...
        if sample_axis is None:
            ...
            assert len(self.sample_shape), (
                "Received sample_axis but self.weights has"
                " no dedicated axis for samples; this"
                " usually implies that sample_shape=[]."
            )

            ndims_x = len(
                get_inducing_shape(x) if isinstance(x, InducingVariables) else x.shape
            )
        ...
        vals = move_axis(
            batch_tensordot(self.weights, feat, axes=[-1, -1], batch_axes=batch_axes),
...
class MultioutputDenseSampler(DenseSampler):
    ...
    def __call__(self, x: tf.Tensor, sample_axis: int = "default", **kwargs):
        ...
        vals = move_axis(
            batch_tensordot(
                self.weights,  # output features go last
                feat,
                axes=[-1, -1],

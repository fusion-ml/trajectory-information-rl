...
from gpflow_sampling.inducing_variables import InducingImages, DepthwiseInducingImages
...
@Kfu.register(SharedIndependentInducingVariables, kernels.SharedIndependent, TensorLike)
def _Kfu_fallback_shared(Z, kern, X, **kwargs):
    return _Kfu_fallback(Z, kern, X, **kwargs)  # Edge-case where L is supressed


def _Kfu_conv2d_fallback(
    feat: InducingImages, kern: Conv2d, Xnew: tf.Tensor, full_spatial: bool = False
):
...
def _Kfu_conv2d(
    ...
    r2 = tf.transpose(
        tf.nn.conv2d(
            input=tf.square(feat.as_images),
            filters=filters,
            strides=[1, 1],
            padding="VALID",
        )
...
def _Kfu_depthwise_conv2d_fallback(
    feat: DepthwiseInducingImages,
...
def _Kfu_depthwise_conv2d(
    ...
    if full_spatial:
        Kxz = tf.reduce_mean(
            tf.reshape(Kxz, list(Kxz.shape[:-1]) + [channels_in, -1]), axis=-2

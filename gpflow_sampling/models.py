    ...
    def predict_f_samples(
        ...
        **kwargs
        ...
        if num_samples is not None:
            assert num_samples == self.paths.sample_shape, ValueError(
                "Requested number of samples does not match path count."
            )
...
class PathwiseGPR(GPR, PathwiseGPModel):
    ...
    def generate_paths(
        ...
        **kwargs
        ...
        if prior is None:
            prior = sampling.priors.random_fourier(
                self.kernel,
                num_bases=num_bases,

...
from gpflow_sampling.utils import conv_ops, swap_axes, move_axis, batch_tensordot
...
class Conv2d(kernels.MultioutputKernel):
    def __init__(
        ...
        if isinstance(weights, str) and weights == "default":  # TODO: improve me
            spatial_out = self.get_spatial_out(
                spatial_in=image_shape,
    ...
    def K(self, X: tf.Tensor, X2: tf.Tensor = None, full_spatial: bool = False):
        ...
        return tf.tensordot(
            tf.linalg.matvec(K, self.weights), self.weights, axes=[-2, 0]
        )
...
class Conv2dTranspose(Conv2d):
    def __init__(self, *args, strides: List = None, dilations: List = None, **kwargs):

        strides = list((1, 1) if strides is None else strides)
        dilations = list((1, 1) if dilations is None else dilations)
        if strides != [1, 1] and dilations != [1, 1]:
            raise NotImplementedError(
                "Tensorflow does not implement transposed"
    ...
    def convolve(
        ...
        conv_kwargs = dict(filters=_filters, padding=padding, output_shape=shape_out)
        ...
        if dilations != [1, 1]:  # TODO: improve me
            ...
            return tf.nn.atrous_conv2d_transpose(
                input, rate=dilations[0], **conv_kwargs
            )
    ...
    def get_patches(self, X, full_spatial: bool = False):
        ...
        return tf.reshape(
            tf.reverse(  # reverse channel-wise patches and reshape
...
class DepthwiseConv2d(Conv2d):
    def __init__(
        ...
        **kwargs,
        ...
        if strides != [1, 1] and dilations != [1, 1]:
            warn(
                f"{self.__class__} does not pass unit tests when strides != [1, 1]"
        ...
        if isinstance(weights, str) and weights == "default":  # TODO: improve me
            ...
            weights = tf.ones(
                [tf.reduce_prod(spatial_out), channels_in], dtype=default_float()
            )
    ...
    def K(self, X: tf.Tensor, X2: tf.Tensor = None, full_spatial: bool = False):
        ...
        K = move_axis(
            tf.linalg.diag_part(move_axis(self.kernel.K(P, P2), P.shape.ndims - 2, -2)),
            -1,
            0,
        )
    ...
    def get_patches(self, X: TensorType, full_spatial: bool = False):
        ...
        depthwise_patches = tf.reshape(
            patches, list(patches.shape[:-1]) + [-1, channels_in]
        )

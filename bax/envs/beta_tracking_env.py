...
DEFAULT_MODEL_INFO_PATH = '/zfsauton/project/public/ichar/BetaTracking/data/beta_only/beta_only_info.pkl'
DEFAULT_START_DATA_PATH = '/zfsauton/project/public/ichar/BetaTracking/data/beta_only/beta_only_data.hdf5'
...
class BetaTrackingModelEnv(EnvModel):
    ...
    def __init__(
        ...
        self.state_space = state_space
        self.include_target_in_state_space = (include_target_in_state_space
                or (target_distribution is not None))
    ...
    def unroll(self, start_states, policy, horizon, actions=None):
        ...
        if self.target_distribution is not None:
            obs[..., -1] = np.tile(
                    self.target_distribution(start_states.shape[0]),
                    horizon + 1).reshape(horizon + 1, start_states.shape[0])
        ...
        env_infos = {'targets': np.ones((horizon, start_states.shape[0])) * self.target
                                if self.target_distribution is None
                                else obs[..., -1]}
    ...
    def multi_step(self, states, actions):
        ...
        nxt_states[:, [i * 2 + 1 for i in range(signal_deltas.shape[1])]] \
                = signal_deltas
    ...
    def _action_to_normd_pinj(self, states, actions):
        ...
        if self.action_is_change:
            ...
            return np.clip(
                pinj_change,
                np.clip(self.scaled_pinj_bounds[0] - curr_pinjs,
                        np.ones(len(curr_pinjs)) * self.scaled_pinj_dt_bounds[0],
                        np.zeros(len(curr_pinjs))),
                np.clip(self.scaled_pinj_bounds[1] - curr_pinjs,
                        np.zeros(len(curr_pinjs)),
                        np.ones(len(curr_pinjs)) * self.scaled_pinj_dt_bounds[1]),
            )
        else:
            pinj_targets = (actions
                * (self.scaled_pinj_bounds[1]
